...
For now, we can begin with setting permissions to the contexts we have created. Let's begin with the Manager context. Our case study dictates that a manager must be able to read and edit salaries. Let's go ahead and implement this in our model. We want to \`allow\` \`read\` and \`allow\` \`write\` permissions on the \`salary\` resource. This means we are telling Sentinel that anyone who is granted the Manager context is "allowed" to "read" salary and "write" "salary". Recall how the Manager context is granted to Danny. This means after these calls we will have successfully implemented a model where Danny as a Manager can \`read\` and \`write\` salaries. You can name a permission as you wish - a good habit is to name them using an idiomatic napspaces as described in the earlier sections. We chose the following names \`lexunit:rolce:read\` and \`lexunit:rolce:write\`. Here's a call to \`allow\` \`lexunit:rolce:read\` permissions on salary for the Manager context

\`\`\`
curl -X PUT "https://sentinelbeta.herokuapp.com/v1/permissions/{MANAGER_CONTEXT_ID}/resources/{SALARY_RESOURCE_ID}" -H "accept: application/json" -H "x-sentinel-tenant: $TENANT" -H "Authorization: bearer $ACCESS_TOKEN" -H "Content-Type: application/json" -d "{ "data": { "attributes": { "name": "lexunit:rolce:read", "permitted": "allow" }, "type": "permission" }}"
\`\`\`

Similarly, let us \`allow\` \`lexunit:rolce:write\` permissions on salary resource for the Manager context. 

\`\`\`
curl -X PUT "https://sentinelbeta.herokuapp.com/v1/permissions/{MANAGER_CONTEXT_ID}/resources/{SALARY_RESOURCE_ID}" -H "accept: application/json" -H "x-sentinel-tenant: $TENANT" -H "Authorization: bearer $ACCESS_TOKEN" -H "Content-Type: application/json" -d "{ "data": { "attributes": { "name": "lexunit:rolce:write", "permitted": "allow" }, "type": "permission" }}"
\`\`\`

With these permissions set, Danny now has \`lexunit:rolce:read\` and \`lexunit:rolce:write\` access to salary resource. Let's make the same call on Employee context. This time we only want \`allow\` \`lexunit:rolce:read\` permissions. The call would look like this: 
  ...
  "data": [
    {
      ...
      "relationships": {
        "permissions": {
          "data": [
            {
              "name": "lexunit:rolce:write",
              "permitted": "allow"
            },
            {
              "name": "lexunit:rolce:read",
              "permitted": "allow"
            }
          ]
        }
...
This response is strictly about the Principal we provided in the request. In this case, it is about Danny. Sentinel is telling us that Danny has \`lexunit:rolce:write\` and \`lexunit:rolce:read\` permissions for Target resource \`salary\`.
...
This response is strictly about the Principal we provided in the request. In this case, it is about Linus. Sentinel is telling us that Linus has only \`lexunit:rolce:read\`  permissions for Target resource \`salary\`. With this, we have a basic idea about storing permissions with Sentinel. Next up, let's look at some advanced use cases.`
