...
import { useFetchContexts } from "../services/useFetchContexts";
import queryString from "query-string";
import { AppContext } from "../layouts/Admin";

const AdminNavbar = (props) => {
  const query = queryString.parse(props.location.search);
  const { contexts } = useFetchContexts(query.user);
  const { state, dispatch } = useContext(AppContext);

  const contextChangeHandler = (e) => {
    console.log(e.target.value);
    dispatch({ type: "UPDATE_CONTEXT", data: e.target.value });
  };
  ...
  return (
    <>
      ...
      <Modal modalClassName="modal-search" isOpen={false}>
